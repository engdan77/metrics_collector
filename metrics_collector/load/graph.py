...
class GarminAppleLoadGraph(BaseLoadGraph):
    ...
    def get_all_graph_methods(
        ...
        return (
            self.graph_monthly_run_count_pace,
    ...
    def graph_monthly_run_count_pace(self) -> plotly.graph_objects.Figure:
        ...
        df = df.resample("M").agg(
            {"avg_speed_running_trip": "max", "running_distance_meters": "count"}
        )
        df.rename(columns={"running_distance_meters": "number_of_runs"}, inplace=True)
        df.astype(float)
        df["number_of_runs"].interpolate(
            method="linear", limit_direction="backward", axis=0, inplace=True
        )
        df = df.ffill().bfill()
        # Build graph
        fig = go.Figure()
        fig.add_trace(
            go.Bar(x=df.index, y=df.number_of_runs, name="Number of runs", yaxis="y")
        )
        fig.add_trace(
            go.Scatter(
                x=df.index,
                y=df.avg_speed_running_trip,
                name="Max minutes/Km",
                yaxis="y2",
            )
        )
        fig.update_layout(
            xaxis=dict(domain=[0.5, 0.5]),
            yaxis=dict(
    ...
    def graph_weekly_distance(self) -> plotly.graph_objs.Figure:
        ...
        fig = px.bar(
            df, y=["walking_km", "running_km"], title="Weekly distance", height=500
        )
        fig.update_layout(
            legend=dict(yanchor="top", y=0.99, xanchor="left", x=0.01),
            legend_title_text=None,
