...
class FooLoadGraph(BaseLoadGraph):
    ...
    def graph_foo(self) -> plotly.graph_objects.Figure:
        df = self.df
        fig = go.Figure()
        fig.add_trace(go.Bar(x=df.index, y=df.running, name="Running distance", yaxis='y'))
        return fig


@pytest.fixture
def load_graph_obj(transform_obj):
    obj = FooLoadGraph(transform_obj, datetime.date(2022, 1, 1), datetime.date(2022, 1, 10))
    return obj
